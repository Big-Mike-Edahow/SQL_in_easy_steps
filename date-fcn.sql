...
INSERT INTO labor_day (date) VALUES ("2025-09-05 12:45:30");

# get the name of the day
SELECT DAYNAME(date) FROM labor_day;

# get the day, month and year date components
SELECT DAYOFMONTH(date), MONTHNAME(date),YEAR(date) FROM labor_day;

# get the hour, minute and second time components
SELECT HOUR(date), MINUTE(date), SECOND(date) FROM labor_day;
